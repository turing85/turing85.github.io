...
[source, docker]
----
FROM eclipse-temurin:17.0.6_10-jdk-alpine AS builder <1>
RUN mkdir /project
WORKDIR /project
COPY . .
RUN ./mvnw package
...
FROM eclipse-temurin:17.0.6_10-jre-alpine AS alpine-runner <3>
COPY \
  --from=builder \ <6>
  --chmod=444 \
...
.Visual representation of the images created. Text in "<<>>" represents the name of a layer.
[#plant-layers]
[plantuml, link=self]
----
...
$ podman build \
  ...
  --tag hello-world:without-target \
  .
[1/5] STEP 1/5: FROM eclipse-temurin:17.0.6_10-jdk-alpine AS builder
...
$ podman build \
  ...
  --tag hello-world:alpine \
  .
[1/3] STEP 1/5: FROM eclipse-temurin:17.0.6_10-jdk-alpine AS builder
...
--> 21f9b27bf70
[3/3] STEP 1/3: FROM eclipse-temurin:17.0.6_10-jre-alpine AS alpine-runner
