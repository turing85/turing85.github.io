...
We have to reinitialize am `ARG` in each stage we want to use it in. And why does printing the value through `echo` not work? Because it is an `ARG`, and ``ARG``s are only available at image build-time. In particular, they are not available in ``ENTRYPOINT``s and ``CMD``s.
...
----
<1> Set the value of `name` ot the value of environment variable `GREETING`. Sets it to `null` if `GREETING` is not set.
